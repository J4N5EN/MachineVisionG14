...
while True:
    ...
    # Press 'space' to capture and use the current frame
    if cv2.waitKey(1) & 0xFF == ord(' '):
        if ret:
            print("Captured.")
        else:
            print("No corners found, nothing captured.")
...
if len(objpoints) > 0:
    ...
    np.savez('calibration_data.npz', ret=ret, camera_matrix=mtx, dist_coeff=dist, rvecs=rvecs, tvecs=tvecs)
